...
func mockRequestUnmarshaller(mr *mockRequest) requestUnmarshaller {
...
func TestQueuedRetry_DropOnPermanentError(t *testing.T) {
	...
	be := newBaseExporter(&defaultExporterCfg, zap.NewNop(), mockRequestUnmarshaller(mockR), WithRetry(rCfg), WithQueue(qCfg))
...
func TestQueuedRetry_DropOnNoRetry(t *testing.T) {
	...
	rCfg := DefaultRetrySettings()
	rCfg.Enabled = false
	be := newBaseExporter(&defaultExporterCfg, zap.NewNop(), nopRequestUnmarshaller(), WithRetry(rCfg), WithQueue(qCfg))
	ocs := newObservabilityConsumerSender(be.qrSender.consumerSender)
	be.qrSender.consumerSender = ocs
	require.NoError(t, be.Start(context.Background(), componenttest.NewNopHost()))
	t.Cleanup(func() {
